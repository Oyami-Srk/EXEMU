...
#define SIZEOF_PERIPHERAL_CONFIG(cfg)                                          \
  (sizeof(cfg) / sizeof(struct peripheral_link_t))
...
char *get_phy_mem_addr() { return VM_RAM; }
...
uint32_t memory_reset(void *base) {
  ...
  log_f("Load ROM AT:%08x, Size:%08x\r\n", ROM_LOAD_ADDR,
        fread(&VM_RAM[ROM_LOAD_ADDR], sizeof(char), rom_file_stat.st_size,
              rom_file));
...
struct peripheral_link_t peripheral_config[] = {{.name = "RAM",
                                                 .mask = ~(RAM_SIZE - 1),
                                                 .prefix = 0x00000000,
                                                 .reg_base = 0x100,
                                                 .reset = memory_reset,
...
#define WARN(...)  do{ if(1){log_f(__VA_ARGS__);} }while(0)
int emu_thread(void *p) {
  cpu = calloc(1, sizeof(struct armv4_cpu_t));
  cpu_init(cpu);

  cpu->peripheral_link = peripheral_config;
  cpu->peripheral_number = SIZEOF_PERIPHERAL_CONFIG(peripheral_config);
  cpu->running = 1002;
  cpu->mmu.reg[0] = 10;
  ...
  for (int i = 0; i < 4096; i++) {
    uint32_t dat = read_word_without_mmu(cpu, cpu->mmu.reg[2] + (i << 2));
    if (dat != 0)
      log_f("%d, %08x\n", i, dat);
  }
  log_f("\n-----------MMU table end--------------\n");
