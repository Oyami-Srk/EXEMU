...
char reg_dump_str[1024];

#define PRINTF(...)  do{ if(DEBUG){log_f(__VA_ARGS__);} }while(0)
#define WARN(...)  do{ if(1){log_f(__VA_ARGS__);} }while(0)
#define REGDUMP(...)  do{ if(1){reg_dump_log_f(__VA_ARGS__);} }while(0)
#define ERROR(...)  do{ if(1){log_f(__VA_ARGS__);} }while(0)
...
void reg_show(struct armv4_cpu_t *cpu)
{
    ...
    clr_reg_dump_text();
    REGDUMP("PC = 0x%x , code = %u\r\n", cpu->reg[CPU_MODE_USER][15], cpu->code_counter);
    REGDUMP("cpsr = 0x%x, %s\n", cpsr(cpu), string_register_mode[cur_mode]);
    REGDUMP("User mode register:");
    for(int i=0; i<16; i++) {
        if(i % 4 == 0) {
            REGDUMP("\n");
        }
        REGDUMP("R%2d = 0x%08x, ", i, cpu->reg[CPU_MODE_USER][i]);
    }
    REGDUMP("\r\nPrivileged mode register\r\n");
    for(int i=1; i<7; i++) {
        REGDUMP("R%2d = 0x%08x, ", 13, cpu->reg[i][13]);
        REGDUMP("R%2d = 0x%08x, ", 14, cpu->reg[i][14]);
